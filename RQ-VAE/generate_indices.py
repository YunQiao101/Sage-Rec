...
output_file = f"{dataset}.index.epoch{args_setting.epoch}.json"
...
labels_2 = {"0":[],"1":[],"2":[], "3":[]}
embs  = [layer.embedding.weight.cpu().detach().numpy() for layer in model.clb_rq.vq_layers]
for idx, emb in enumerate(embs):
    centers, label = constrained_km(emb)
...
for d in tqdm(data_loader):
    x,y, emb_idx = d[0], d[1],d[2]
    x = x.to(device)
    y = y.to(device)
    
    # indices = model.get_indices(d, use_sk=False)
    indices,indices_2 = model.get_indices(x,y, labels,labels_2,use_sk=False)
...
# # save SID
# torch.save(all_indices,os.path.join(output_dir,f'mm_ID_SID_4096_num{args_setting.epoch}_0.pt'))
# torch.save(all_indices_2,os.path.join(output_dir,f'mm_clb_SID_4096_num{args_setting.epoch}_0.pt'))
#
# # save codebook embedding
# emb = model.rq.get_code()
# torch.save(emb,os.path.join(output_dir,f'ID_SID_code_4096_num{args_setting.epoch}_0.pt'))
# emb = model.clb_rq.get_code()
# torch.save(emb,os.path.join(output_dir,f'clb_SID_code_4096_num{args_setting.epoch}_0.pt'))
#
#
